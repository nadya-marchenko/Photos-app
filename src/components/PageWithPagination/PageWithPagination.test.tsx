...
beforeEach(() => {
  axios.get = jest.fn(() => Promise.resolve<any>({ data: fakeData }));
});

afterEach(cleanup);

describe('<PageWithPagination />', () => {
  it('should display title, grid, search and pagination', async () => {
    let wrapper;
    ...
    await act(async () => {
      ...
      expect(
        wrapper.find('[data-testid="not-loading"]').at(0).props().isLoading,
      ).toEqual(true);
      expect(
        wrapper.find('[data-testid="not-loading"]').at(0).props().photos,
      ).toEqual([]);
    });
